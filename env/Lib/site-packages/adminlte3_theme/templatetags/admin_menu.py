...
class _Menu:
    ...
    def add(self, label, link="", icon="", id_="", parent=""):
        ...
        if parent != "":
            ...
            self.parents.append(
                {id_: {"label": label, "link": link, "icon": icon, "children": []}}
            )
    ...
    def render(self, context, menus=None):
        ...
        if len(menus) <= 0:
            ...
            if request.path == reverse("admin:index"):
                r = (
                    ...
                    + reverse("admin:index")
                    + '" class="nav-link active"><i class="nav-icon fas fa-tachometer-alt"></i> <p>Home</p></a></li>'
                )
            else:
                r = (
                    '<li class="nav-item"><a href="'
                    + reverse("admin:index")
                    + '" class="nav-link"><i class="nav-icon fas fa-tachometer-alt"></i> <p>Home</p></a></li>'
                )
        ...
        for group in menus:
            ...
            if len(group[key]["children"]) > 0:
                r += (
                    '<li class="nav-item has-treeview"><a href="#" class="nav-link">%s <span>%s</span><span class="pull-right-container"><i class="fas fa-angle-left right"></i></span></a><ul class="treeview-menu">\n'
                ...
                if request.path == reverse("admin:index"):
                    r += (
                        '<li class="nav-item"><a href="%s" class="nav-link">%s <p>%s</p></a></li>\n'
    ...
    def admin_apps(self, context, r):
        ...
        for app in context["available_apps"]:
            if str(app["app_url"]) in request.path:
                r += (
                    '<li class="nav-item has-treeview menu-open"><a href="#" class="nav-link active"><i class="nav-icon fas fa-edit"></i> <p>%s</p><p><i class="fas fa-angle-left right"></i></p></a><ul class="nav nav-treeview">\n'
                    % (app["name"])
                )
            else:
                r += (
                    '<li class="nav-item has-treeview"><a href="#" class="nav-link"><i class="nav-icon fas fa-edit"></i> <p>%s</p><p><i class="fas fa-angle-left right"></i></p></a><ul class="nav nav-treeview">\n'
                    % (app["name"])
                )
            ...
            for model in app["models"]:
                ...
                if request.path == url:
                    r += (
                        '<li class="nav-item"><a href="%s" class="nav-link active">%s %s</a></li>'
                        % (url, icon, model["name"])
                    )
                else:
                    r += (
                        '<li class="nav-item"><a href="%s" class="nav-link">%s %s</a></li>'
                        % (url, icon, model["name"])
                    )
